...
/*
 *  自己添的这些方法  目前不可用  因为一个结构体的参数实在太多了----感觉根本找不到啊 
 */

+ (NSString *)get_destination{
    NSString * routerDes = nil;
    NSMutableArray *routerArray = [Route_Info getRoutes];
    for (int i = 0; i < (int)[routerArray count]; i++) {
        Route_Info *router = (Route_Info*)[routerArray objectAtIndex:i];
        routerDes = [router getDestination];
    }
    return routerDes;
}

+ (NSString *)get_netmask{
    NSString * routerNet = nil;
    NSMutableArray *routerArray = [Route_Info getRoutes];
    for (int i = 0; i < (int)[routerArray count]; i++) {
        Route_Info *router = (Route_Info*)[routerArray objectAtIndex:i];
        routerNet = [router getNetmask];
    }
    return routerNet;
}
